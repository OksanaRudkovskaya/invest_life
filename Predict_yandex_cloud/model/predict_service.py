...
from src.predict.models import DataItem
from model.model_lstm import LSTMModel
from settings import SCALERS_PATH, MODEL_PATH
...
class PredictService:
    ...
    def predict_last_row(self, data: DataItem, window=30):
        ...
        if ticker not in self.scalers:
            raise ValueError(f"Нет скалера для тикера '{ticker}'. Модель не обучалась на этом тикере.")
